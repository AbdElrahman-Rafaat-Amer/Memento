...
class ReminderEditorMapper @Inject constructor() {
    ...
    fun toDomain(uiState: ReminderEditorUiState, id: Long = 0L): Reminder {
        return Reminder(
            id = id,
            title = uiState.title,
            date = requireNotNull(uiState.date) {
                "Date cannot be null"
            }.toEpochDay(),
            time = requireNotNull(uiState.time) {
                "Time cannot be null"
